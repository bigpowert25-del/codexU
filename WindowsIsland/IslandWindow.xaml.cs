...
public partial class IslandWindow : Window
{
    ...
    private bool _shouldAutoHide = true;
    ...
    public void ShowNotification(NotificationData data)
    {
        Dispatcher.Invoke(() =>
        {
            ...
            _shouldAutoHide = true;
            Expand(hasBody);
        });
    }

    public void ShowSnapshot(CodexUSnapshot snapshot)
    {
        Dispatcher.Invoke(() =>
        {
            _currentAppId = null;
            ...
            TimeText.Text = snapshot.RefreshedAt.ToLocalTime().ToString("HH:mm");

            AppIcon.Visibility = Visibility.Collapsed;
            AvatarFallback.Visibility = Visibility.Visible;
            AvatarInitial.Text = "C";
            AvatarFallback.Background = AccentBrush("Codex");

            _hideTimer?.Stop();
            Visibility = Visibility.Visible;
            NativeInterop.ForceTopMost(this);
            _shouldAutoHide = false;
            Expand(hasBody: true);
        });
    }
    ...
    private void Expand(bool hasBody)
    {
        ...
        {
            _isFullyOpen = true;
            if (_shouldAutoHide && !PillBorder.IsMouseOver) StartHideTimer();
            else TriggerHover();
        });
    }
    ...
    private void PillBorder_MouseLeave(object sender, System.Windows.Input.MouseEventArgs e)
    {
        ...
        if (_shouldAutoHide) StartHideTimer();
    }
